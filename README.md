...
Self-hosted AI workflows break when the workstation goes down. Cloud APIs and frontier model API services are the usual fallback, but they cost money, leak data, and require internet. A phone with a local GGUF model gives you:
...
Telegram / Discord
  ...
  └────┬─────┘
       ...
       │
       ├── Tier 2: Phone (llama.cpp, 3-4B OSS model) ── failover
       │           SSH tunnel: VPS:18081 → phone:8081
       │
       └── Tier 3: Cloud API (frontier model) ─────────── cloud fallback
```
...
3. Both unreachable → fall back to a cloud-hosted frontier model API
...
- GGUF model (any 3-4B class open-source model, e.g. Qwen 3-4B Q4_K_M)
...
# Download a 3-4B class open-source model (example: Qwen 3-4B)
...
(Any 3-4B parameter open-source GGUF model works — Qwen, Gemma, Phi, etc.)
...
|--------|-------|---------|--------|------------|
| Snapdragon 8 Elite (phone) | Qwen 3-4B Q4 | OpenCL (Adreno 830) | ~9.4 t/s | ~2.6 t/s |
| Snapdragon 8 Elite (phone) | Qwen 3-4B Q4 | CPU-only | ~20 t/s | ~20 t/s |
